...
class db{
    ...
    public function __construct($config=array()){
        ...
        $this->config['DB_USER'] = isset($config['DB_USER'])?$config['DB_USER']:'root';//数据库用户名
        $this->config['DB_PWD'] = isset($config['DB_PWD'])?$config['DB_PWD']:'HIBSOSjpDXL5uvFjaIkqq';//数据库密码
        $this->config['DB_PORT'] = isset($config['DB_PORT'])?$config['DB_PORT']:3306;//数据库端口，mysql默认是3306，一般不需要修改
        $this->config['DB_NAME'] = isset($config['DB_NAME'])?$config['DB_NAME']:h5;//数据库名
