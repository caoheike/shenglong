...
case "send": $nickname = $_SESSION['username'];
    ...
    if(get_query_val("fn_ban", "id", array("roomid" => $_SESSION['roomid'], 'userid' => $_SESSION['userid'])) != ""){
        echo json_encode(array('success' => false, 'msg' => '您因被禁言故无法发言与投注!'));
        break;
    }elseif(!wordkeys($content)){
        echo json_encode(array('success' => false, 'msg' => '您发送的内容内含有屏蔽词!'));
        break;
    }
    if($type == 'U3'){
    ...
    if(get_query_val("fn_setting", "setting_ischat", array("roomid" => $_SESSION['roomid'])) == 'open' && !$co && !$fenshuchange){
        echo json_encode(array('success' => false, 'msg' => "♤投注格式错误或游戏已封盘♤"));
        break;
    }else{
        echo json_encode(array("success" => true, "content" => $content));
        insert_query("fn_chat", array("username" => $nickname, 'content' => $content, 'addtime' => date('H:i:s'), 'game' => $_COOKIE['game'], 'headimg' => $headimg, 'type' => $type, 'userid' => $_SESSION['userid'], 'roomid' => $_SESSION['roomid']));
...
if ((int) $zym_9 < (int) $zym_5) {
    ...
    if ($zym_6 == '庄' || $zym_6 == '闲' || $zym_6 == '和' || $zym_6 == '庄对' || $zym_6 == '闲对' || $zym_6 == '任意对') {
        ...
        $touzhu = true;
        用户_下分($userid, $zym_5);
        insert_query("fn_bjlorder", array("term" => $addQihao, 'userid' => $userid, 'username' => $_SESSION['username'], 'headimg' => $_SESSION['headimg'], 'content' => $zym_6, 'money' => $zym_5, 'addtime' => 'now()', 'roomid' => $_SESSION['roomid'], 'status' => '未结算', 'jia' => $zym_8));
        continue;
    }
}
if ($zym_2 != "") {
