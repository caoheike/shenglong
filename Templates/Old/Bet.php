...
<body>
 ...
 if($game == 'xy28' || $game == 'jnd28'){
    ...
    ?>
				...
				<?php }elseif($game == 'cqssc' || $game=='azxy5'){
    ?>
						<thead>
							<tr>
								<th>期号</th>
     ...
     select_query($table, '*', "`userid` = '{$_SESSION['userid']}' and `status` = '未结算' and `roomid` = '{$_SESSION['roomid']}'");
    while($con = db_fetch_array()){
        $cons[] = $con;
        ?>
									<tr>
...
?>
    ...
    ?>
     ...
     select_query($table, '*', "`roomid` = '{$_SESSION['roomid']}' and `userid` = '{$_SESSION['userid']}' and `status` != '未结算' and `addtime` like '" . date('Y-m-d') . "%'");
    $all_m = 0;
    $all_z = 0;
    while($con = db_fetch_array()){
        $cons[] = $con;
