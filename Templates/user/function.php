...
function getxia2($userid) {
    ...
    $my_ss="and addtime>'$time1' and addtime<'$time2'";
    $my_ss1="and time>'$time1' and time<'$time2'";
    select_query("fn_user", '*', "roomid = {$_SESSION['roomid']} and agent = '{$userid}'");
    while ($con = db_fetch_array()) {
        $cons[] = $con;
    }
    foreach ($cons as $con) {
        $l = (int) get_query_val('fn_order', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0)  $my_ss");
        $pcl = (int) get_query_val('fn_pcorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $bjll = (int) get_query_val('fn_bjlorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $sscl = (int) get_query_val('fn_sscorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $mtl = (int) get_query_val('fn_mtorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $jsscl = (int) get_query_val('fn_jsscorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $jssscl = (int) get_query_val('fn_jssscorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0) $my_ss");
        $z = get_query_val('fn_order', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $pcz = get_query_val('fn_pcorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $bjlz = get_query_val('fn_bjlorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $sscz = get_query_val('fn_sscorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $mtz = get_query_val('fn_mtorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $jsscz = get_query_val('fn_jsscorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $jssscz = get_query_val('fn_jssscorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0 $my_ss");
        $y = $z - $l;
        $bjly = $bjlz - $bjll;
        $pcy = $pcz - $pcl;
        $sscy = $sscz - $sscl;
        ...
        $yk = $y + $pcy + $sscy + $mty + $jsscy + $jssscy+$bjly;
        $yk = sprintf("%.2f", substr(sprintf("%.3f", $yk), 0, -2));
        $allyk += $yk;
        $liushui = $l + $pcl + $sscl + $mtl + $jsscl + $jssscl+$bjll;
        $allliu += $liushui;
        $s = get_query_val('fn_upmark', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` = '已处理' $my_ss1");
        $alls += $s;
...
$bjll = (int)get_query_val('fn_bjlorder', 'sum(`money`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and (`status` > 0 or `status` < 0)");
...
$bjlz = get_query_val('fn_bjlorder', 'sum(`status`)', "roomid = {$_SESSION['roomid']} and userid = '{$con['userid']}' and `status` > 0");
$y = $z - $l;
$pcy = $pcz - $pcl;
$sscy = $sscz - $sscl;
$mty = $mtz - $mtl;
